...
impl FileReaderWriter {
    ...
    pub fn get_size(&mut self, boot: &uefi::prelude::BootServices) -> usize {
        let mut buf = [0x0u8; 0x0];
        let size = match self.file.get_info::<FileInfo>(&mut buf) {
            Err(e) => {
                let buffer_len = e.data().unwrap();
                let buffer = boot
                    .allocate_pool(MemoryType::BOOT_SERVICES_DATA, buffer_len)
                    .unwrap_success();
                ...
                match self.file.get_info::<FileInfo>(buffer_slice) {
                    Ok(info) => {
                        let size = info.unwrap().file_size();
